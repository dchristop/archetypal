...
and has full control to all external and internal parameteres of it.
...
4. freqstable is a frequncy table for all candidate initial points found

5. iterations are the number of main iterations done by algorithm

6. time is the seconds ellapsed for the entire process
...
The archetypes are indeed on the outer boundary, more precicely they form a principal convex hull of data points:
...
```{r checkB, echo=TRUE}
BB=aa$B
yy=check_Bmatrix(B=BB,print.details = TRUE)
# yy$used_rows
# yy$used_weights
```
...
yy2=check_Bmatrix(aa2$B,print.details = TRUE)
```

The same solution as before, but with 44 instead of 63 ierations!
...
(Data frames are equal at least untill the 16th decimal point.)
...
irows=sapply(yy3$used_rows, function(x){x[1]});irows
